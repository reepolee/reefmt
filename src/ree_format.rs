...
pub(crate) fn format_ree_content(content: &str, wrap_width: usize, collapse_blocks: bool, max_members: usize) -> String {
    let ast_output = crate::ree_parser::format_ree(content, wrap_width);
    format_script_blocks(&ast_output, wrap_width, collapse_blocks, max_members)
}

fn format_script_blocks(content: &str, wrap_width: usize, collapse_blocks: bool, max_members: usize) -> String {
    ...
    while let Some(script_start) = remaining.find("<script") {
        ...
        if let Some(tag_end) = remaining[script_start..].find('>') {
            ...
            if let Some(script_end) = remaining[tag_close..].find("</script>") {
                ...
                let formatted = format_script_content(script_content, wrap_width, collapse_blocks, max_members);
...
fn format_script_content(content: &str, wrap_width: usize, collapse_blocks: bool, max_members: usize) -> String {
...
pub(crate) fn format_ree_file(path: &Path, mode: crate::format::Mode, wrap_width: usize, collapse_blocks: bool, max_members: usize) -> bool {
    ...
    let write_content = format_ree_content(&normalized, wrap_width, collapse_blocks, max_members);
...
mod tests {
    ...
    fn format_ree_content_idempotent() {
        ...
        let result = format_ree_content(src, 120, true, 3);
    ...
    fn idempotent_non_ascii_in_script_comment() {
        ...
        let pass1 = format_ree_content(src, 120, true, 3);
        let pass2 = format_ree_content(&pass1, 120, true, 3);
