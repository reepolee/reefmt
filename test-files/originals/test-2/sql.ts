...
export async function search_records(
	...
	try {
		...
		const query =
			`SELECT * FROM v_images ${where} ORDER BY ${sort_field} ${valid_direction.toUpperCase()}, id ${valid_direction.toUpperCase()} LIMIT ? OFFSET ?`;
