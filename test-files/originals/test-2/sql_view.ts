...
export async function get_all_records_view(
	...
	try {
		...
		return await timed_query("v_images", "get_all_records_view", async () => {
			...
			const data_query =
				`SELECT * FROM v_images ${where} ORDER BY ${sort_field} ${query_direction.toUpperCase()}, id ${query_direction.toUpperCase()} LIMIT ?`;
			params.push(limit);
			...
			if (search) {
				...
				const count_query = `SELECT COUNT(*) as count FROM v_images WHERE ${get_fulltext_clause()}`;
			...
			if (!search) {
				...
				const count_query = `SELECT COUNT(*) as count FROM v_images${count_where}`;
		...
		console.error("Error fetching records from view v_images:", error);
