 ...
 */
export function hard_clone(s: string): string {
	return new TextDecoder().decode(new TextEncoder().encode(String(s)));
}
...
export function ext_of(filename: string): string {
	...
	if (
		...
		|| clean.includes("releaselock")
		|| clean.includes("stream reader")
	) {
...
export function parse_crop(form_data: FormData): {
	left: number;
	top: number;
	width: number;
	height: number;
...
export async function save_upload_to_temp(form_data: FormData): Promise<Upload> {
	...
	log_warn("editor", "save_upload_to_temp:normalized", {
		client_name,
		file_name_after_normalize: file.name,
		original_name,
	});
	...
	log_warn("editor", "save_upload_to_temp:temp path", {
		temp_path,
		file_name: file.name,
		original_name,
	});
	...
	log_warn("editor", "save_upload_to_temp:after write", {
		file_name: file.name,
		original_name,
		temp_path,
	});
...
export async function cleanup(...paths: (string | undefined)[]) {
