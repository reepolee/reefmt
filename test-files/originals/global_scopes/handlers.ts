...
export async function get_global_scopes_edit(req: BunRequest): Promise<Response> {
	...
	const [module_options, table_options, user_options] = await Promise.all([
		get_available_modules(),
		get_available_tables(),
		get_users_select_options(),
	]);

	const translations_url = `/system/translations/${encodeURIComponent(ns_for_translations)}/scopes/edit?from=${
		encodeURIComponent(ns_for_translations)
	}`;
...
export async function post_global_scopes_test_scope(req: BunRequest): Promise<Response> {
	try {
		...
		if (!where_clause) {
			return new Response(JSON.stringify({ error: "where_clause is required" }), {
				status: 400,
				headers: { "Content-Type": "application/json" },
			});
		}
		...
		if (Number.isNaN(test_user_id)) {
			return new Response(JSON.stringify({ error: "test_user_id is required" }), {
				status: 400,
				headers: { "Content-Type": "application/json" },
			});
		}
		...
		return new Response(JSON.stringify({ resolved_clause }), {
			status: 200,
			headers: { "Content-Type": "application/json" },
		});
	} catch (error) {
...
export async function post_global_scopes_bulk_delete(req: BunRequest): Promise<Response> {
	if (!enable_delete) {
		return new Response(JSON.stringify({ error: "Bulk delete is disabled." }), {
			status: 403,
			headers: { "Content-Type": "application/json" },
		});
	}
	...
	try {
		...
		if (!Array.isArray(ids) || ids.length === 0) {
			return new Response(JSON.stringify({ error: msg.bulk_delete_no_ids || "No records selected." }), {
				status: 400,
				headers: { "Content-Type": "application/json" },
			});
		}
