...
function entity_path(id?: number | string): string {
...
export async function post_global_scopes_edit(req: BunRequest): Promise<Response> {
		...
		if (scope_record) {
			...
			for (const lang of languages) {
				await upsert_translation(
					...
					namespace,
					key_path,
					scope_record.display_name || scope_record.scope_key,
				);
			}
	...
	if (action === "delete") {
		if (!enable_delete) {
			return new Response(JSON.stringify({ error: "Delete is disabled." }), {
				status: 403,
		...
		try {
			...
			const [module_options, table_options] = await Promise.all([
	...
	try {
		...
		const error_key = error instanceof Error && error.message.toLowerCase().includes("duplicate entry")
			? "duplicate_key"
