...
describe("load_and_validate_redirects", () => {
	...
	test("rejects non-array input", () => {
		expect(() => load_and_validate_redirects({} as unknown)).toThrow(RedirectsError);
	});
	...
	test("rejects duplicate `from` (including slash variants)", () => {
		const input = [
			{ from: "/resume", to: "/a.pdf" },
			{ from: "/resume/", to: "/b.pdf" },
		];
	...
	test("accepts an empty array", () => {
		expect(load_and_validate_redirects([])).toEqual([]);
	});
});
...
describe("check_collisions_and_validate_targets", () => {
	...
	test("fails when `from` collides with a static asset", () => {
		...
		expect(() => check_collisions_and_validate_targets(redirects, dist, new Set(), static_assets)).toThrow(
			/collides with a static asset/,
		);
	});
});
