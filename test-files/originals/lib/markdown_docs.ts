 ...
 *
 * Called from scripts/build.ts and scripts/dev.ts after Bun.markdown.html()
...
export function process_docs_markdown(
	raw_html: string,
	styles: MarkdownStyles = default_markdown_styles,
	...
	html = html.replace(/<pre><code class="language-(\w+)">([\s\S]*?)<\/code><\/pre>/g, (_, lang, encoded) => {
	...
	html = html.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g, (_, encoded) => {
		...
		return `<pre${cls(styles.pre)}><code class="hljs">${highlighted}</code></pre>`;
	});
	...
	if (styles.table_wrapper) {
		html = html.replaceAll("<table>", `<div class="${styles.table_wrapper}"><table${cls(styles.table)}>`);
