...
<a name="building-a-language-picker"></a>

## Building a Language Picker

<media-frame label="Screenshot — language picker" ratio="16/9"></media-frame>
...
`config/supported_languages.ts` is read once on import. `lib/i18n.ts` walks `routes/` and builds the translation tree at module load. `lib/route_map.ts` builds the localised-URL lookup tables in `server.ts`:
