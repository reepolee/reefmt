 ...
 *
 * Two-phase contract with `scripts/build.ts`:
...
export function load_and_validate_redirects(input: unknown): Redirect[] {
	...
	for (let i = 0; i < input.length; i++) {
		...
		if (typeof from !== "string") {
			throw new RedirectsError(`${label}: \`from\` must be a string`);
		}
		if (!from.startsWith("/")) {
		...
		if (status !== undefined && status !== 301 && status !== 302) {
			throw new RedirectsError(`${label}: \`status\` must be 301 or 302 (got ${String(status)})`);
		}
		...
		result.push({
			from: normalized,
			to,
			status: status as 301 | 302 | undefined,
		});
	}
 ...
 */
export function check_collisions_and_validate_targets(
	redirects: Redirect[],
	dist_dir: string,
	generated_routes: Set<string>,
	static_asset_paths: Set<string>,
): void {
	for (const r of redirects) {
		...
		if (!is_external(r.to)) {
			...
			const candidates = looks_like_file
				? [join(dist_dir, r.to)]
				: [join(dist_dir, r.to, "index.html"), join(dist_dir, r.to)];

			const found = candidates.some((p) => existsSync(p));
			if (!found) {
				throw new RedirectsError(
					`Redirect from="${r.from}" to="${r.to}": target does not exist in dist/. Checked: ${candidates.join(", ")}`,
				);
			}
		}
	}
