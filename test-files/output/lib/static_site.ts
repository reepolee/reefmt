 ...
 * present.
 */
export function build_static_route_map(
		...
		for (const lang of languages) {
			...
			for (const segment of segments) {
				...
				if (
					...
					typeof candidate.route_name === "string" &&
 ...
 */
export function collect_page_files(
	public_dir: string,
	languages: readonly string[],
	extensions: readonly string[] = ["ree", "md"],
): string[] {
...
export function parse_frontmatter(text: string): { data: FrontMatter; body: string } {
	...
	if (!close_match || close_match.index === undefined) {
		return { data: {}, body: text };
	}
