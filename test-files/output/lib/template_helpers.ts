...
export function is_current(url: string, request_url?: string): string {
	...
	const current =
		request_url === url_norm || request_url.startsWith(url_norm + "/") || request_url.startsWith(url_norm + "?");
...
const PADDED_TIME: Intl.DateTimeFormatOptions = {
	hour: "2-digit",
	minute: "2-digit",
};

const PADDED_DATETIME: Intl.DateTimeFormatOptions = {
	...PADDED_DATE,
	...PADDED_TIME,
};
...
export function tags(val: string, color_class: string = "pill-default"): string {
	...
	return val
		...
		.map((t) => t.trim())
		.filter(Boolean)
		.map((t) => pill(t, PILL_TAG_LAYOUT + " " + color_class))
		.join(" ");
}
...
export function create_default_helpers(data: any = {}): TemplateHelpers {
	...
	return {
		...
		js_date_to_locale_string: (date_input: string | Date, l: string = locale) =>
			js_date_to_locale_string(date_input, l),
		js_time_to_locale_string: (date_input: string | Date, l: string = locale) =>
			js_time_to_locale_string(date_input, l),
		js_datetime_to_locale_string: (datetime_input: string | Date, l: string = locale) =>
			js_datetime_to_locale_string(datetime_input, l),
		js_timestamp_to_locale_string: (timestamp_input: string | Date, l: string = locale) =>
			js_timestamp_to_locale_string(timestamp_input, l),
		js_date_to_iso_string,
		js_datetime_to_iso_string,
		js_timestamp_to_iso_string,
		display_currency: (val: number, l: string = locale, hide_zero = false, symbol = "€") =>
			display_currency(val, l, hide_zero, symbol),
