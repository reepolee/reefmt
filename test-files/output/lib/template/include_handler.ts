 ...
 */
export async function include_resolved_handler(
	deps: IncludeHandlerDeps,
	currentName: string,
	includeName: string,
	...
	if (info.kind === "template") {
		...
		if (!(await file_exists(p))) {
			throw new Error(`Included file not found: ${p}`);
		}
		...
		if (extname(p) === deps.ext) {
			...
			const rtInclude = (name: string, data: Record<string, any>) =>
				include_resolved_handler(deps, includeName, name, data);
