 ...
 *
 * Takes a preprocessed template string (with custom elements, comments, and
 * spread shorthands already resolved) and compiles it into an async function
 ...
 */
function parseCall(s: string): { name: string; dataExpr: string } | null {
 ...
 */
function emit_text(text: string): string {
 ...
 */
function emit_code_block(rawJS: string): string {
	return `${rawJS}\n`;
}
...
function handle_layout(trimmedCall: string): LayoutInfo {
	const parsed = parseCall(trimmedCall);
	if (!parsed) {
		throw new Error(`Invalid layout syntax: "{#${trimmedCall}}". Expected {#layout('path'[, data])}`);
	}
...
function emit_include(trimmedCall: string): string {
	const parsed = parseCall(trimmedCall);
	if (!parsed) {
		throw new Error(`Invalid include syntax: "{#${trimmedCall}}". Expected {#include('path'[, data])}`);
	}
...
function emit_with_open(trimmedContent: string, state: CompileState): string {
	...
	if (!expr) {
		throw new Error(`Invalid #with syntax: "{#with ${trimmedContent}}". Expected "{#with <expr> }"`);
	}
	const id = state.eachCounter++;
...
function emit_if_open(trimmedContent: string, state: CompileState): string {
	...
	if (!cond) {
		throw new Error(`Invalid #if syntax: "{#if ${trimmedContent}}". Expected "{#if <condition> }"`);
	}
...
function emit_else(state: CompileState): string {
	...
	if (current.hasElse) {
		throw new Error("Multiple {:else} in the same block are not allowed");
	}
 ...
 *
 * @param processedTemplate - Template string after custom element, comment,
 *                            and spread shorthand preprocessing.
...
export function compile_to_code(processedTemplate: string, slotFns: CompiledFn[]): { code: string; fn: CompiledFn } {
	...
	processedTemplate = processedTemplate.replace(
		/\u0000R\u0000([^\u0000]+)\u0000(\d+)\u0000([\s\S]*?)\u0000/g,
		(_match, tagName, _slotId, propsObj) => {
	...
	const combinedPattern = /\{\{([\s\S]*?)\}\}|\{([~=#:/])\s*([\s\S]*?)\}|\u0000J\u0000([\s\S]*?)\u0000\u0000/g;
...
${code}
			...
			__escape: (x: any) => string,
			__include: (n: string, d: Record<string, any>) => Promise<string>,
			__rtInclude: (n: string, d: Record<string, any>) => Promise<string>,
	...
	const compiled = async (
		props: Record<string, any>,
		escape: (x: any) => string,
		boundInclude: (n: string, d: Record<string, any>) => Promise<string>,
		rtInclude: (n: string, d: Record<string, any>) => Promise<string>,
		currentName: string,
	) => {
