...
export function resolve_md_for_lang(
	public_dir: string,
	rel_path: string,
	lang: string,
 ...
 *
 * Shared by scripts/build.ts (static build) and scripts/dev.ts (dev server) so
...
export async function resolve_route_records(
	...
	if (wants_loader && existsSync(data_full)) {
		...
		if (route.source === "loader") {
			console.warn(`   ⚠  ${data_rel} does not export load_records() — falling back to markdown`);
		}
	}

	return collect_records(public_dir, route_dir, lang, page_files, route.sort ?? "date_desc");
}
