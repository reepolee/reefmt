...
class TemplateEngine {
	...
	compile_to_code(template: string): { code: string; fn: CompiledFn } {
		...
		const { template: processedTemplate, slotFns } = preprocess_template(
			template,
			this.viewsDir,
			this.ext,
	...
	/** Include partial template (views-root relative; used by layout) */
	...
	/** Render template by name (views-root relative, without extension) */
	async render(name: string, props: Record<string, any> = {}): Promise<string> {
		...
		if (this.cache) {
			this.compiledCache[resolvedName] = compiledFn;
		}
	...
	/** Render from string directly (no file). Relative includes resolve from views root. */
	...
	/** Write rendered output to file (Bun) */
