...
Conditions are also resilient at render time: if the expression _throws_ — say you reach through a property that turned out to be `undefined` — the engine catches it and treats the condition as falsy rather than crashing the whole render. So `{#if props.user.profile.verified }` renders the `{:else}` branch (or nothing) when `props.user.profile` is missing, instead of erroring. This is a safety net, not an invitation to skip optional chaining — `props.user?.profile?.verified` still expresses intent more clearly and is the recommended form.
...
The `{#each}` block has its own `{:else}` branch for the "list is empty" case, so you rarely need `{#if props.records.length > 0 }` around an each block — write the `{:else}` inline instead. See [Loops](/reeweb/docs/ree-templates/loops).
