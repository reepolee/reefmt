...
/**
 * Data loader for the /reeweb/docs index page.
...
export async function load_template_data(): Promise<Record<string, any>> {
	...
	const docs_dir = join(public_dir, "reeweb", "docs");

	const sidebar_groups = build_sidebar_groups(docs_dir);
	const canonical_path = "/reeweb/docs";

	return {
		title: "Reeweb documentation",
...
function build_sidebar_groups(docs_dir: string): SidebarGroup[] {
	...
	for (const section of reeweb_docs_sidebar) {
		...
		if (existsSync(folder_path)) {
			...
			for (const entry of entries) {
				...
				const canonical = template_to_canonical(`reeweb/docs/${section.slug}/${entry}`);
