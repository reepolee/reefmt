...
/**
 * Data loader for the /ree-templates-vscode/docs index page.
...
export async function load_template_data(): Promise<Record<string, any>> {
	...
	const docs_dir = join(public_dir, "ree-templates-vscode", "docs");

	const sidebar_groups = build_sidebar_groups(docs_dir);
	const canonical_path = "/ree-templates-vscode/docs";

	return {
		title: "ree Templates extension documentation",
...
function build_sidebar_groups(docs_dir: string): SidebarGroup[] {
	...
	for (const section of ree_templates_docs_sidebar) {
		...
		if (existsSync(folder_path)) {
			...
			for (const entry of entries) {
				...
				const canonical = template_to_canonical(`ree-templates-vscode/docs/${section.slug}/${entry}`);
