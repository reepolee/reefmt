...
/**
 * Data loader for the /reemerge/docs index page.
...
export async function load_template_data(): Promise<Record<string, any>> {
	...
	const docs_dir = join(public_dir, "reemerge", "docs");

	const sidebar_groups = build_sidebar_groups(docs_dir);
	const canonical_path = "/reemerge/docs";

	return {
		title: "reemerge documentation",
...
function build_sidebar_groups(docs_dir: string): SidebarGroup[] {
	...
	for (const section of reemerge_docs_sidebar) {
		...
		if (existsSync(folder_path)) {
			...
			for (const entry of entries) {
				...
				const canonical = template_to_canonical(`reemerge/docs/${section.slug}/${entry}`);
