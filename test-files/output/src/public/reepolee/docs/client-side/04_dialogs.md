...
<media-frame label="Screenshot — confirm dialog with blurred backdrop" ratio="16/9"></media-frame>
...
- **Trigger button uses `commandfor` + `command="show-modal"`** — the browser opens the dialog when the button is clicked. No `onclick`, no JS, no signals.
