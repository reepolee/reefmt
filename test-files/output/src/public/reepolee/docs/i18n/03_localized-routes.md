...
The `route_name` key in a translation file tells the route map how to localise that segment. The `route_name` value is automatically slugified via `slugify()` in `lib/route_aliases.ts` — it transliterates Unicode characters to ASCII (e.g., `ü` → `ue`, `š` → `s`), lowercases, and replaces whitespace with hyphens. The Slovenian translation of `routes/system/translations/sl.json`:
...
<media-frame label="Screenshot — language-mismatch dialog" ratio="16/9"></media-frame>
...
2. **Builds the localised path** in the target language.
