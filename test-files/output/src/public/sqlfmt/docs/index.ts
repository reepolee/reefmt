...
/**
 * Data loader for the /sqlfmt/docs index page.
...
export async function load_template_data(): Promise<Record<string, any>> {
	...
	const docs_dir = join(public_dir, "sqlfmt", "docs");

	const sidebar_groups = build_sidebar_groups(docs_dir);
	const canonical_path = "/sqlfmt/docs";

	return {
		title: "sqlfmt documentation",
...
function build_sidebar_groups(docs_dir: string): SidebarGroup[] {
	...
	for (const section of sqlfmt_docs_sidebar) {
		...
		if (existsSync(folder_path)) {
			...
			for (const entry of entries) {
				...
				const canonical = template_to_canonical(`sqlfmt/docs/${section.slug}/${entry}`);
