...
import { create_toast_cookie, get_lang_from_request, localized_url, translated_from_request } from "$lib/helpers";
import { reload_all_translations, loaded_translations } from "$lib/i18n";
...
if (path.includes(base_path()) && !path.includes("/edit")) return path;
 ...
 catch (e) { console.warn("Invalid referer URL:", referer, e); }
return null;
}
...
if (return_url_from_form?.includes?.(bp)) { redirect_url = return_url_from_form; }
 else {
...
for (const lang of languages) { await upsert_translation(lang, namespace, key_path, scope_record.display_name || scope_record.scope_key); }
await reload_all_translations();
reload_route_maps(loaded_translations, routes, languages);
			
const toast_cookie = make_toast("toast-scope-translations", { message: `Scope translations created for "${scope_record.scope_key}".`, type: "green", duration: 3000 });
