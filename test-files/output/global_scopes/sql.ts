...
export async function get_global_scopes_select_options(): Promise<Options[]> {
	try {
		return await timed_query("global_scopes", "get_select_options", async () => {
			const records =
				await db`SELECT id as option_value, table_name as option_text FROM global_scopes ORDER BY table_name ASC LIMIT 50`;
...
export async function search_records(
	...
	try {
		...
		const valid_direction = ["asc", "desc"].includes(sort_direction?.toLowerCase() || "")
		...
		return await timed_query("global_scopes", "search_records", async () => {
			...
			const data_query =
				`SELECT * FROM global_scopes ${where} ORDER BY ${sort_field} ${valid_direction.toUpperCase()}, id ${valid_direction.toUpperCase()} LIMIT ? OFFSET ?`;
...
export async function create_record(record: Omit<Record, "id">): Promise<Record> {
	try {
		return await timed_query("global_scopes", "create_record", async () => {
			const insert_result =
				await db`INSERT INTO global_scopes (module_code, feature_name, table_name, scope_key, display_name, where_clause, sort_order, is_default) VALUES (${record.module_code}, ${
					record.feature_name || ""
				}, ${record.table_name}, ${record.scope_key}, ${record.display_name}, ${record.where_clause}, ${record.sort_order}, ${record.is_default})`;
...
export async function update_record(id: number, record: Omit<Record, "id">): Promise<Record | undefined> {
	try {
		return await timed_query("global_scopes", "update_record", async () => {
			await db`UPDATE global_scopes SET module_code = ${record.module_code}, feature_name = ${
				record.feature_name || ""
			}, table_name = ${record.table_name}, scope_key = ${record.scope_key}, display_name = ${record.display_name}, where_clause = ${record.where_clause}, sort_order = ${record.sort_order}, is_default = ${record.is_default} WHERE id = ${id}`;
			const records = await db`SELECT * FROM global_scopes WHERE id = ${id} LIMIT 1`;
