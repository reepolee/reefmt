...
for (let i = 0; i < process.argv.length; i++) {
	...
	if (arg === "--dir" || arg === "--dist") {
		...
		port = Number(process.argv[++i]) || port;
...
console.log(`\n  🖥️ Preview server ready at \x1b[32mhttp://localhost:${port}/\x1b[0m`);
console.log(`  📂 Serving: ${dist_dir}`);
console.log(`  🌐 Languages: ${all_langs.length > 0 ? all_langs.join(", ") : "none"}`);

if (has_root_lang) {
	console.log(`   ↳ Default language "${default_language}" (${default_lang_name}) served at / (no prefix)`);
