...
export async function get_all_records(): Promise<Record[]> { try { return await timed_query("global_scopes", "get_all_records", async () => {
...
export async function get_global_scopes_select_options(): Promise<Options[]> { try { return await timed_query("global_scopes", "get_select_options", async () => {
...
export async function get_record_by_id(id: number): Promise<Record | undefined> { try { return await timed_query("global_scopes", "get_record_by_id", async () => {
...
export async function search_records(search: string = "", offset: number = 0, limit: number = 20, order_by: string = "id::asc", scope_clause: string = "", filter_clauses: {
	clause: string;
	params: any[];
}[] = []): Promise<{ records: Record[]; total: number; }> { try {
...
if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sort_field)) { return search_records(search, offset, limit, "id::asc", scope_clause, filter_clauses); }
...
export async function create_record(record: Omit<Record, "id">): Promise<Record> { try { return await timed_query("global_scopes", "create_record", async () => {
...
export async function update_record(id: number, record: Omit<Record, "id">): Promise<Record | undefined> { try { return await timed_query("global_scopes", "update_record", async () => {
...
export async function delete_record(id: number): Promise<boolean> { try { return await timed_query("global_scopes", "delete_record", async () => {
