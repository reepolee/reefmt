...
import { get_user_by_id, get_users_select_options } from "$routes/system/users/sql";
...
const [translated, module_options, table_options, user_options] = await Promise.all([
		translated_from_request(req, import.meta.dir),
		get_available_modules(),
		get_available_tables(),
		get_users_select_options()
	]);
...
const [record, translated] = await Promise.all([get_record_by_id(id), translated_from_request(req, import.meta.dir)]);
...
export async function post_global_scopes_test_scope(req: BunRequest): Promise<Response> { try {
const body = await req.json();
const where_clause: string = body.where_clause?.trim?.() || "";
const test_user_id: number = parseInt(body.test_user_id, 10);
	
if (!where_clause) { return new Response(JSON.stringify({ error: "where_clause is required" }), { status: 400, headers: { "Content-Type": "application/json" } }); }
	
if (Number.isNaN(test_user_id)) { return new Response(JSON.stringify({ error: "test_user_id is required" }), { status: 400, headers: { "Content-Type": "application/json" } }); }
	
const user_record = await get_user_by_id(test_user_id);
if (!user_record) { return new Response(JSON.stringify({ error: "User not found" }), { status: 404, headers: { "Content-Type": "application/json" } }); }
...
return new Response(JSON.stringify({ resolved_clause }), { status: 200, headers: { "Content-Type": "application/json" } });
}
 catch (error) {
console.error("Error previewing scope:", error);
return new Response(JSON.stringify({ error: "Preview failed" }), { status: 500, headers: { "Content-Type": "application/json" } });
} }
...
if (!Array.isArray(ids) || ids.length === 0) { return new Response(JSON.stringify({ error: msg.bulk_delete_no_ids || "No records selected." }), { status: 400, headers: {
...
if (deleted) {
sql_log({ s: "Delete", t: "global_scopes", id: String(id) }, req);
return { deleted: true };
			}
...
for (const result of results) { if (result.status === "fulfilled") { if (result.value.deleted) deleted_count++;
 else error_count++; }
 else {
...
return new Response(JSON.stringify({ deleted: deleted_count, errors: error_count, message }), { status: 200, headers: {
			"Content-Type": "application/json",
			"Set-Cookie": toast_cookie.toString()
		} });
	}
 catch (err) {
