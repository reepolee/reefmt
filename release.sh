...
#
# reefmt is pure Rust (no C/native deps), so every target cross-compiles cleanly
# from macOS:
...
if [ "$new_commits" -gt 0 ] && [ "$force_skip_bump" = false ]; then
	...
	echo "═══ reefmt release $new_version (all targets) ═══"
	...
	echo "═══ reefmt release $version (all targets) ═══"
